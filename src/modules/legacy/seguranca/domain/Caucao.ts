...
export class Caucao {
    ...
    static criar(props: CriarCaucaoProps): Resultado<Caucao, Error> {
        ...
        return ResultadoUtil.resultados(
            [setPaymentId, setValor, setStatus, setCheckoutUrl, setMpResponse],
            instancia,
        );
    }
    ...
    podeSerProcessada(): boolean {
        return (
            this.status === StatusCaucao.PAGA ||
            this.status === StatusCaucao.PROCESSANDO
        );
    }
    ...
    private setPaymentId(paymentId: string): Resultado<void, Error> {
        if (!paymentId) {
            return ResultadoUtil.falha(
                new Error('Payment ID não pode ser vazio'),
            );
        }
    ...
    private setValor(valor: number): Resultado<void, Error> {
        if (valor <= 0) {
            return ResultadoUtil.falha(
                new Error('Valor da caução deve ser maior que zero'),
            );
        }
