...
export class VerificacaoResidencia {
    ...
    static criar(
        props: CriarVerificacaoResidenciaProps,
    ): Resultado<VerificacaoResidencia, Error> {
    ...
    static carregar(
        props: VerificacaoResidenciaProps,
        id: string,
    ): VerificacaoResidencia {
    ...
    private aprovar(moderadorId?: string, observacoes?: string): void {
        if (this.props.status !== ModeracaoStatus.EM_ANALISE) {
            throw new Error(
                'A verificação deve estar em análise para ser aprovada',
            );
        }
    ...
    private rejeitar(
        ...
        if (this.props.status !== ModeracaoStatus.EM_ANALISE) {
            throw new Error(
                'A verificação deve estar em análise para ser rejeitada',
            );
        }
    ...
    cancelar(): void {
        if (
            ...
            throw new Error(
                'Não é possível cancelar uma verificação já concluída',
            );
        }
    ...
    private setComprovanteUrl(comprovanteUrl: string): Resultado<void, Error> {
        if (!comprovanteUrl) {
            return ResultadoUtil.falha(
                new Error('URL do comprovante é obrigatória'),
            );
        }
    ...
    private setObservacoesModerador(
        observacoesModerador?: string,
    ): Resultado<void, Error> {
