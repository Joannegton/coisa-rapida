...
import {
    ...
    ModeracaoStatus,
} from '../models/VerificacaoResidencia.model';
import {
    ResultadoAssincrono,
    ResultadoUtil,
    ServicoExcecao,
...
export class VerificacaoResidenciaRepository {
    ...
    async salvar(
        ...
        try {
            ...
            return ResultadoUtil.falha(
                new ServicoExcecao(
                    error?.message || 'Erro ao criar verificação',
                ),
            );
        }
    }
    ...
    async atualizar(
        ...
        try {
            const verificacaoModel = await VerificacaoResidenciaModel.findOne({
                where: { id: props.id },
            });
            if (!verificacaoModel) {
                return ResultadoUtil.falha(
                    new ServicoExcecao(
                        'Verificação não encontrada para atualização',
                    ),
                );
            }
            ...
            return ResultadoUtil.falha(
                new ServicoExcecao(
                    error?.message || 'Erro ao atualizar verificação',
                ),
            );
        }
    }
    ...
    async buscarPorId(
        id: string,
    ): ResultadoAssincrono<VerificacaoResidencia | null, ServicoExcecao> {
        try {
            ...
            return ResultadoUtil.falha(
                new ServicoExcecao(
                    error?.message || 'Erro ao buscar verificação',
                ),
            );
        }
    }
    ...
    async listarPendentes(
        ...
        try {
                ...
                const usuarioDomain = Usuario.carregar(
                    {
                        ...
                        endereco: enderecoDomain,
                        enderecoVerificado:
                            verificacao.usuario.enderecoVerificado,
                    },
                    verificacao.usuario.id,
                );
            ...
            return ResultadoUtil.falha(
                new ServicoExcecao(
                    error?.message || 'Erro ao listar verificações pendentes',
                ),
            );
        }
    }

    async buscarPendentePorUsuarioId(
        usuarioId: string,
    ): ResultadoAssincrono<VerificacaoResidencia | null, ServicoExcecao> {
        try {
            const verificacao = await VerificacaoResidenciaModel.findOne({
                where: {
                    ...
                    status: ModeracaoStatus.EM_ANALISE,
                },
                relations: ['usuario', 'usuario.endereco'],
            });
            ...
            if (!verificacao) {
                return ResultadoUtil.falha(
                    new Error(
                        'Nenhuma verificação pendente encontrada para este usuário',
                    ),
                );
            }
            ...
            return ResultadoUtil.falha(
                new ServicoExcecao(
                    error?.message || 'Erro ao buscar verificação pendente',
                ),
            );
        }
    }
