...
export class MercadoPagoTransferService {
    ...
    gerarInstrucoesTransferenciaManual(params: DadosTransferenciaManual): {
        ...
        this.logger.log(
            `Gerando instruções para transferência de R$ ${params.valor.toFixed(2)} para ${params.destinatario}`,
        );
     ...
     */
    async reembolsar(
        params: ReembolsoParams,
    ): ResultadoAssincrono<ReembolsoResponse, ServicoExcecao> {
        try {
            this.logger.log(
                `Iniciando reembolso do pagamento ${params.paymentId}`,
            );
            ...
            this.logger.log(
                `Reembolso realizado com sucesso: ${response.data.id}`,
            );
            ...
            return ResultadoUtil.falha(
                new ServicoExcecao(
                    `Erro ao reembolsar valor: ${error.response?.data?.message || error.message}`,
                ),
            );
        }
    }
