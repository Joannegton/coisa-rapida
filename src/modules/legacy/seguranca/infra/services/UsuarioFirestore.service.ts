...
export class UsuarioFirestoreService {
    ...
    async buscarPorId(
        id: string,
    ): ResultadoAssincrono<UsuarioFirestore | null, ServicoExcecao> {
        try {
            ...
            return ResultadoUtil.falha(
                new ServicoExcecao(
                    error?.message || 'Erro ao buscar usuário no Firestore',
                ),
            );
        }
    }

    async listarAdmins(): ResultadoAssincrono<
        UsuarioFirestore[],
        ServicoExcecao
    > {
        try {
            ...
            return ResultadoUtil.falha(
                new ServicoExcecao(
                    error?.message || 'Erro ao listar admins no Firestore',
                ),
            );
        }
    }
    ...
    async atualizarUsuario(
        ...
        try {
            ...
            if (dados.criadoEm) {
                updateData.criadoEm = admin.firestore.Timestamp.fromDate(
                    dados.criadoEm,
                );
            }
            if (dados.atualizadoEm) {
                updateData.atualizadoEm = admin.firestore.Timestamp.fromDate(
                    dados.atualizadoEm,
                );
            }
            ...
            return ResultadoUtil.falha(
                new ServicoExcecao(
                    error?.message || 'Erro ao atualizar usuário no Firestore',
                ),
            );
        }
    }
