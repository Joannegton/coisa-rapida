...
export class NotificacaoService {
    async enviar(
        props: NotificacaoProps,
    ): ResultadoAssincrono<string, ServicoExcecao> {
        try {
            const notificationSaveResult = await admin
                .firestore()
            ...
            const userDoc = await admin
                .firestore()
                .collection('usuarios')
                .doc(props.destinatarioId)
                .get();

            if (!userDoc.exists) {
                return ResultadoUtil.falha(
                    new ServicoExcecao('Usuário destinatário não encontrado'),
                );
            }
            ...
            if (!fcmToken) {
                return ResultadoUtil.falha(
                    new ServicoExcecao('Usuário não possui token FCM'),
                );
            }
            ...
            try {
                ...
                if (
                    ...
                    return ResultadoUtil.falha(
                        new ServicoExcecao(
                            'Token FCM inválido ou expirado. Token removido.',
                        ),
                    );
                }
                throw fcmError;
            ...
            return ResultadoUtil.falha(
                new ServicoExcecao(
                    error?.message || 'Erro ao enviar notificação',
                ),
            );
        }
    }
