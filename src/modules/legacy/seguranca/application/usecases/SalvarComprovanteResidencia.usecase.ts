...
import {
    ...
    TipoNotificacao,
} from '../../infra/repositories/Notificacao.repository';
...
export class SalvarComprovanteResidenciaUseCase {
    ...
    ): ResultadoAssincrono<string, ComprovanteResidenciaExceptions> {
        try {
            const usuarioFirebaseResult =
                await this.usuarioFirestoreService.buscarPorId(props.usuarioId);
            if (
                usuarioFirebaseResult.ehFalha() ||
                !usuarioFirebaseResult.valor
            ) {
                return ResultadoUtil.falha(
                    new ServicoExcecao('Usuário não encontrado'),
                );
            }

            const verificacoesPendentes =
                await this.verificacaoRepository.buscarPendentePorUsuarioId(
                    props.usuarioId,
                );
            if (verificacoesPendentes.ehSucesso()) {
                return ResultadoUtil.falha(
                    new ServicoExcecao('Já existe uma verificação pendente'),
                );
            }

            // Upload do comprovante para Cloudinary
            const UploadResult =
                await this.cloudinaryService.uploadNoCloudinary({
            ...
            if (UploadResult.ehFalha()) {
                return ResultadoUtil.falha(
                    UploadResult.erro ||
                        new ServicoExcecao('Erro no upload do comprovante'),
                );
            }

            const usuarioResult = Usuario.criar(
                usuarioFirebaseResult.valor,
                props.usuarioId,
            );
            ...
            if (verificacaoDomain.ehFalha()) {
                return ResultadoUtil.falha(
                    verificacaoDomain.erro ||
                        new ServicoExcecao(
                            'Erro ao criar verificação de residência',
                        ),
                );
            }

            // Criar verificação
            const comprovanteResidenciaResult =
                await this.verificacaoRepository.salvar(
                    verificacaoDomain.valor!,
                );

            if (comprovanteResidenciaResult.ehFalha()) {
                return ResultadoUtil.falha(
                    comprovanteResidenciaResult.erro ||
