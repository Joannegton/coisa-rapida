...
export class VerificarCodigoSMSUseCase {
    ...
    async execute(
        ...
        try {
            ...
            const atualizarUsuarioFirebase =
                await this.usuarioFirestoreService.atualizarUsuario(usuarioId, {
                    telefone: telefoneInternacional,
                });

            if (atualizarUsuarioFirebase.ehFalha()) {
                return ResultadoUtil.falha(
                    atualizarUsuarioFirebase.erro ||
                        new Error('Falha ao atualizar usuário no Firebase'),
                );
            }
            ...
            return ResultadoUtil.falha(
                error.message || 'Erro ao verificar código SMS',
            );
        }
    }
}
