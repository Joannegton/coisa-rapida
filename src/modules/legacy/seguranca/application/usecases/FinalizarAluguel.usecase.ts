...
export class FinalizarAluguelUseCase {
    ...
    async executar(
        ...
        const valorLiquidoLocador =
            aluguel.valor!.calcularValorLiquidoLocador();
        ...
        const resultadoTransferencias =
            await this.definirEstrategiaTransferencia(
                aluguel.valor!,
                taxaApp,
                indenizacao,
            );
        ...
        const salvarAluguelResult = await this.aluguelRepository.salvar(
            aluguel.valor!,
        );
    ...
    private async definirEstrategiaTransferencia(
        ...
        if (aluguel.caucao && aluguel.caucao.estaPaga()) {
            ...
            const transferenciaComCaucaoResult =
                await this.executarTransferenciasComCaucao(
                    aluguel,
                    taxaApp,
                    indenizacao,
                );
            ...
            const transferenciaSemCaucaoResult =
                await this.executarTransferenciasSemCaucao(
                    aluguel,
                    taxaApp,
                    indenizacao,
                );
    ...
    private async executarTransferenciasComCaucao(
        ...
        this.logger.log(
            `💰 Taxa da plataforma retida: R$ ${taxaApp.toFixed(2)}`,
        );
        this.logger.log(`📋 Cenário: Caução já paga - Reembolso do excedente`);

        const caucao = aluguel.caucao!;
        const retornoLocatario =
            aluguel.calcularRetornoLocatarioComIndenizacao(indenizacao);
        ...
        if (retornoLocatario.valor! > 0 && caucao.paymentId) {
            ...
            const adicionarTransferenciaLocatario =
                aluguel.adicionarTransferencia(transferenciaLocatario.valor!);
            if (adicionarTransferenciaLocatario.ehFalha()) {
                return ResultadoUtil.falha(
                    adicionarTransferenciaLocatario.erro!,
                );
            }
            ...
            this.logger.log(
                `✅ Reembolso para locatário realizado: ID ${reembolsoLocatarioId}`,
            );
        } else if (retornoLocatario.valor! <= 0) {
            this.logger.log(
                'Sem valor a reembolsar para locatário (caução consumida totalmente)',
            );
        }
    ...
    private async executarTransferenciasSemCaucao(
        ...
        this.logger.log(
            `💰 Locatário deve pagar: R$ ${aluguel.valorAluguel.toFixed(2)}`,
        );
        this.logger.log(
            `📋 Cenário: Sem caução - Transferência manual ao locador`,
        );
        ...
        if (valorParaLocador > 0) {
            this.logger.log(
                `📤 Gerando instruções para transferência de R$ ${valorParaLocador.toFixed(2)} ao locador ${aluguel.locador.nome}`,
            );
            ...
            const transferenciaLocador = Transferencia.criar({
                ...
                tipo:
                    indenizacao > 0
                        ? TipoTransferencia.INDENIZACAO
                        : TipoTransferencia.PAGAMENTO_LOCADOR,
            ...
            const adicionarTransferenciaLocador =
                aluguel.adicionarTransferencia(transferenciaLocador.valor!);
            ...
            const instrucoesTransfer =
                this.transferService.gerarInstrucoesTransferenciaManual({
                    valor: valorParaLocador,
            ...
            const marcarAguardandoManual =
                aluguel.marcarTransferenciaAguardandoManual(
            ...
            this.logger.log(
                `📝 Instruções geradas: ${instrucoesTransfer.instrucoes}`,
            );
