...
export class CriarAluguelUseCase {
    ...
    async executar(
        props: CriarAluguelDto,
    ): ResultadoAssincrono<CriarAluguelResponse, Error> {
        this.logger.log('Iniciando criação de aluguel');

        try {
            const statusInicial = props.caucao
                ? StatusAluguel.AGUARDANDO_CAUCAO
                : StatusAluguel.EM_ANDAMENTO;
            ...
            if (aluguel.ehFalha()) {
                this.logger.error(
                    `Erro ao criar aluguel: ${aluguel.erro?.message}`,
                );
            ...
            if (props.caucao) {
                ...
                const mpResult =
                    await this.mpCheckoutService.criarPreferenciaCaucao({
                ...
                if (mpResult.ehFalha()) {
                    this.logger.error(
                        `Erro ao criar preferência MP: ${mpResult.erro?.message}`,
                    );
                ...
                if (caucaoResult.ehFalha()) {
                    this.logger.error(
                        `Erro ao criar caução: ${caucaoResult.erro?.message}`,
                    );
                    return ResultadoUtil.falha(caucaoResult.erro!);
                }

                // Adicionar caução ao aluguel
                const adicionarCaucaoResult = aluguel.valor!.adicionarCaucao(
                    caucaoResult.valor!,
                );
                if (adicionarCaucaoResult.ehFalha()) {
                    this.logger.error(
                        `Erro ao adicionar caução ao aluguel: ${adicionarCaucaoResult.erro?.message}`,
                    );
                ...
                this.logger.log(
                    `Criando preferência de pagamento do aluguel de R$ ${props.valorAluguel}`,
                );

                const mpResult =
                    await this.mpCheckoutService.criarPreferenciaAluguel({
                        aluguelId: aluguel.valor!.id,
                        valorAluguel: props.valorAluguel,
                        itemNome: props.item.nome,
                        locatarioEmail: props.locatario.email,
            ...
            const salvarResult = await this.aluguelRepository.salvar(
                aluguel.valor!,
            );
            if (salvarResult.ehFalha()) {
                this.logger.error(
                    `Erro ao salvar aluguel: ${salvarResult.erro?.message}`,
                );
            ...
            this.logger.error(
                `Erro inesperado ao criar aluguel: ${(error as Error).message}`,
            );
