...
export class ListarVerificacoesPendentesUseCase {
    ...
    async execute(
        limite: number,
    ): ResultadoAssincrono<VerificacaoPendenteDto[], ServicoExcecao> {
        try {
            // Buscar verificações pendentes
            const verificacoesResult =
                await this.verificacaoRepository.listarPendentes(limite);
            if (verificacoesResult.ehFalha()) {
                return ResultadoUtil.falha(
                    verificacoesResult.erro ||
                        new ServicoExcecao('Erro ao listar verificações'),
                );
            }

            // Buscar dados dos usuários
            const verificacoesComUsuario = verificacoesResult.valor?.map((v) =>
                v.toDto(),
            );
