...
export class AluguelRepository {
  async salvar(aluguel: Aluguel): ResultadoAssincrono<Aluguel, Error> {
    try {
      ...
      const caucaoModel = aluguel.caucao ? this.caucaoDomainToModel(aluguel.caucao) : undefined;
      const transferenciasModels = aluguel.transferencias && aluguel.transferencias.length > 0
        ? aluguel.transferencias.map(t => this.transferenciaDomainToModel(t))
        : [];
  ...
  async buscarPorPaymentId(mpPaymentId: string): ResultadoAssincrono<Aluguel, Error> {
    try {
        const model = await AluguelModel.findOne({
            where: { mpPaymentId },
            relations: ['caucao', 'transferencias'],
  ...
  async buscarPorStatus(status: StatusAluguel): ResultadoAssincrono<Aluguel[], Error> {
    try {
  ...
  private toDomain(aluguelModel: AluguelModel): Aluguel {
    const caucaoDomain = aluguelModel.caucao ? Caucao.carregar({
    ...
    }, aluguelModel.caucao.id) : undefined;

    const transferenciasDomin: Transferencia[] = [];
    if (aluguelModel.transferencias && aluguelModel.transferencias.length > 0) {
  ...
  private transferenciaDomainToModel(transferencia: Transferencia): TransferenciaModel {
