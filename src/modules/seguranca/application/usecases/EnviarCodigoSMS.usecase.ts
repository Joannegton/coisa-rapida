...
export class EnviarCodigoSMSUseCase {
  ...
  async execute(props: EnviarCodigoSMSProps): ResultadoAssincrono<string, Error> {
    ...
    const resultado = await this.twilioService.enviarCodigoVerificacao(telefoneInternacional);

    if (resultado.ehFalha()) {
        return ResultadoUtil.falha(resultado.erro || new Error('Falha ao enviar código de verificação'));
    }
