...
import { EnderecoDto } from "../application/queries/ListarVerificacoesPendentes.usecase";
...
export class Endereco {
  ...
  // Métodos de negócio
  atualizarCoordenadas(latitude: number, longitude: number): Resultado<void, Error> {
    if (latitude < -90 || latitude > 90) {
      return ResultadoUtil.falha(new Error('Latitude deve estar entre -90 e 90'));
    }
    if (longitude < -180 || longitude > 180) {
      return ResultadoUtil.falha(new Error('Longitude deve estar entre -180 e 180'));
    }
  ...
  atualizarEndereco(parcial: Partial<EnderecoProps>): Resultado<void, Error> {
    ...
    if (parcial.estado && parcial.estado.length !== 2) {
      return ResultadoUtil.falha(new Error('Estado deve ter 2 caracteres'));
    }
