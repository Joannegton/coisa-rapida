...
import { Transferencia, TransferenciaDto, TipoTransferencia, StatusTransferencia } from "./Transferencia";
...
type CriarAluguelProps = Omit<AluguelProps, 'indenizacao' | 'mpPaymentId' | 'transferencias' | 'caucao' | 'criadoEm' | 'finalizadoEm'>;
...
export class Aluguel {
  ...
  static criar(props: CriarAluguelProps): Resultado<Aluguel, Error> {
    ...
    const resultadoValorAluguel = instancia.setValorAluguel(props.valorAluguel);
    const resultadoTaxaAppPercentual = instancia.setTaxaAppPercentual(props.taxaAppPercentual);
    const setStatus = instancia.setStatus(props.status);

    return ResultadoUtil.resultados([resultadoItem, resultadoLocatario, resultadoLocador, resultadoValorAluguel, resultadoTaxaAppPercentual, setStatus], instancia);
  }
  ...
  calcularRetornoLocatarioComIndenizacao(indenizacao: number): Resultado<number, Error> {
    if (this.caucao && this.caucao.valor < indenizacao) {
        return ResultadoUtil.falha(new Error('Indenização não pode ser maior que o valor da caução'));
    }
  ...
  atualizarStatusPagamentoCaucao(status: StatusCaucao, mpPayment: PagamentoStatusResponse): void {
  ...
  definirMetodoPagamentoCaucao(metodoPagamento: MetodoPagamento): void {
    ...
    private setLocatario(locatario: ParticipanteAluguel): Resultado<void, Error> {
    ...
    private setValorAluguel(valorAluguel: number): Resultado<void, Error> {
        if (valorAluguel <= 0) {
            return ResultadoUtil.falha(new Error('Valor do aluguel deve ser maior que zero'));
        }
        this.props.valorAluguel = valorAluguel;
        return ResultadoUtil.sucesso();
    }

    private setTaxaAppPercentual(taxaAppPercentual: number): Resultado<void, Error> {
        if (taxaAppPercentual < 0 || taxaAppPercentual > 1) {
            return ResultadoUtil.falha(new Error('Taxa do app deve estar entre 0 e 1'));
        }
    ...
    private setIndenizacao(indenizacao?: number): Resultado<void, Error> {
        if (indenizacao && indenizacao < 0) {
            return ResultadoUtil.falha(new Error('Indenização não pode ser negativa'));
        }
    ...
    private setTransferencias(transferencias: Transferencia[]): Resultado<void, Error> {
        if (!transferencias) {
            return ResultadoUtil.falha(new Error('Transferências não podem ser nulas'));
        }
        this.props.transferencias = transferencias;
        return ResultadoUtil.sucesso();
    }

    adicionarTransferencia(transferencia: Transferencia): Resultado<void, Error> {
        const transferencias: Transferencia[] = this.transferencias || [];
        for (const t of this.props.transferencias) {
            if (t.id === transferencia.id) {
                return ResultadoUtil.falha(new Error('Transferência já existe neste aluguel'));
            }
   ...
   */
  atualizarStatusTransferencia(transferenciaId: string, novoStatus: StatusTransferencia): Resultado<void, Error> {
    const transferencia = this.props.transferencias.find(t => t.id === transferenciaId);
    if (!transferencia) {
      return ResultadoUtil.falha(new Error(`Transferência ${transferenciaId} não encontrada neste aluguel`));
    }
   ...
   */
  marcarTransferenciasComoConcluida(transferenciaId: string, mpTransferenciaId?: number, mpRefundId?: number): Resultado<void, Error> {
    const transferencia = this.props.transferencias.find(t => t.id === transferenciaId);
    if (!transferencia) {
      return ResultadoUtil.falha(new Error(`Transferência ${transferenciaId} não encontrada`));
    }
   ...
   */
  marcarTransferenciaComoFalhada(transferenciaId: string, errorMessage: string): Resultado<void, Error> {
    const transferencia = this.props.transferencias.find(t => t.id === transferenciaId);
    if (!transferencia) {
      return ResultadoUtil.falha(new Error(`Transferência ${transferenciaId} não encontrada`));
    }
   ...
   */
  marcarTransferenciaAguardandoManual(transferenciaId: string, instrucoes: string): Resultado<void, Error> {
    const transferencia = this.props.transferencias.find(t => t.id === transferenciaId);
    if (!transferencia) {
      return ResultadoUtil.falha(new Error(`Transferência ${transferenciaId} não encontrada`));
    }
   ...
   */
  obterTransferenciaPorId(transferenciaId: string): Transferencia | undefined {
    ...
    obterTransferenciasPendentes(): Transferencia[] {
        return this.props.transferencias.filter(t => t.status === StatusTransferencia.PENDENTE);
    }
