 ...
 *
 * Em produção, use Redis para ambiente distribuído.
 *
 * @example
 * // Incrementar tentativa
 * const tentativas = await this.cacheService.incrementar('usuario:123:enviar-sms', 60);
 *
 * // Verificar se está bloqueado
 * const bloqueado = await this.cacheService.estaBloqueado('usuario:123');
 *
 * // Bloquear usuário
 * await this.cacheService.bloquear('usuario:123', 900); // 15 minutos
 */
@Injectable()
export class AuditoriaCacheService {
    private readonly logger = new Logger(AuditoriaCacheService.name);
